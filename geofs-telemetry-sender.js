...
(function () {
  ...
  function readAircraftName() {
    // Try to get from aircraftRecord first (most reliable)
    const record = globalThis.geofs?.aircraft?.instance?.aircraftRecord;
    const name = record?.name || record?.title;
    if (typeof name === 'string' && name.trim()) return name.replace(/,/g, ' ');

    // Fallback candidates
    const candidates = [
      globalThis.geofs?.aircraftManager?.currentAircraft?.name,
      globalThis.geofs?.aircraft?.name,
      globalThis.geofs?.aircraftInstance?.name
    ];
    for (const c of candidates) {
      if (typeof c === 'string' && c.trim()) return c.replace(/,/g, ' ');
    }
    return 'Unknown';
  }
  ...
  function buildTelemetryLine() {
    ...
    const aircraftName = readAircraftName().replace(/,/g, ' ');  // sanitize: no commas in CSV
    ...
    // 33:    apOn
    // 34:    aircraftName(str) — LAST, safe from internal commas
    return [
